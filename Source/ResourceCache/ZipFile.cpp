      ...
      else
      {
        ...
        for (int j = 0; j < fh.fnameLen; j++)
          if (pfh[j] == '/')
            pfh[j] = '\\';
#endif
        char fileName[fh.fnameLen];
        memcpy(fileName, pfh, fh.fnameLen);
        fileName[fh.fnameLen]=0;
//        _strlwr_s(fileName, fh.fnameLen);
...
QString ZipFile::GetFilename(int i)  const
{
    ...
    if (i >=0 && i < m_nEntries)
    {
      char pszDest[m_papDir[i]->fnameLen];
...
bool ZipFile::ReadLargeFile(int i, void *pBuf, void progressCallback(int, bool &))
{
    ...
    stream.avail_in = (uInt)h.cSize;
    stream.next_out = (Bytef*)pBuf;
    stream.avail_out = (128 * 1024); //read 128k at time of h.ucSize
    stream.zalloc = (alloc_func)0;
    stream.zfree = (free_func)0;
    ...
    if (f)
    {
      ...
      else
      {
        for (int i = 0; i < zip.GetNumFiles(); i++)
        {
          ...
          printf("File \"%s\" (%d unsigned chars): ", fname, len);
