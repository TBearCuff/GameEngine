...
{
    Q_ASSERT(m_state == RUNNING || m_state == PAUSED);
    m_state = SUCCEEDED;
}
...
inline void Process::Pause(void)
{
    if (m_state == RUNNING)
        ...
        qWarning("Attempting to pause a process that isn't running");
}
...
inline void Process::UnPause(void)
{
    if (m_state == PAUSED)
        ...
        qWarning("Attempting to unpause a process that isn't paused");
}
