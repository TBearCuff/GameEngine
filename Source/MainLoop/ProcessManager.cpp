...
unsigned int ProcessManager::UpdateProcesses(unsigned long deltaMs)
{
    ...
    while (it != m_processList.end()) {
        ...
        //save the iterator and increment the old one in case we need to remove the process from the list
        ...
        //give the process an update tick if it's still running
        ...
        if(pCurrProcess->IsDead())
        {
            ...
            default:
                qWarning("A dead process had an unidentified state");
            }
